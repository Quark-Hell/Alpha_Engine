...
namespace Render::WindowsManager::AnomalyEngine {
    ...
    void OpaqueShader::ApplyShadersSettings(Render::WindowsManager::AnomalyEngine::Camera *camera) {
        if (camera == nullptr) {
            Core::Logger::LogError("Camera was null: "  + std::string(__FILE__) + ":" + std::to_string(__LINE__));
            return;
        }
        if (camera->GetParentObject() == nullptr) {
            Core::Logger::LogError("Camera parent object was null: "  + std::string(__FILE__) + ":" + std::to_string(__LINE__));
            return;
        }
